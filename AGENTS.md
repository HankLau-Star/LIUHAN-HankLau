...
- `wrangler.jsonc` 默认只定义 Cloudflare Worker、D1 数据库和静态资产；R2 `MEDIA` 是所有者主动开通计费后才添加的可选素材桶。
- 后台默认使用所有者自己的 GitHub OAuth App；只有 `ADMIN_GITHUB_LOGINS` 中的账号可以建立签名会话。`GITHUB_OAUTH_CLIENT_SECRET` 与 `SESSION_SECRET` 只能作为 Cloudflare Secret 保存，不能提交到仓库。
