...
test("default content contains the complete portfolio structure", () => {
  ...
  assert.equal(defaultSiteContent.spiritualAssets[0].assetType, "text");
  assert.equal(defaultSiteContent.physicalAssets[0].assetType, "object");
  assert.equal(defaultSiteContent.workAssets[0].mediaUrl, "");
});
...
test("content normalization blocks unsafe links and limits collections", () => {
  const content = normalizeSiteContent({
    ...
    spiritualAssets: Array.from({ length: 120 }, (_, index) => ({
      ...
      assetType: "text",
      mediaUrl: index === 0 ? "javascript:alert(4)" : "https://media.example.com/asset.pdf",
  ...
  assert.equal(content.spiritualAssets[0].mediaUrl, "");
...
test("the content console mirrors the three frontend worlds and includes a private asset library", async () => {
  ...
  assert.match(admin, /文字作品/);
  assert.match(admin, /音乐作品/);
...
test("owner-controlled Cloudflare hosting uses free D1, owner-only GitHub OAuth, and optional R2", async () => {
  ...
  assert.match(uploadRoute, /90 \* 1024 \* 1024/);
  assert.match(uploadRoute, /text\/markdown/);
  assert.match(uploadRoute, /audio\/flac/);
  assert.match(uploadRoute, /video\/quicktime/);
  assert.match(uploadRoute, /storageConfigured/);
